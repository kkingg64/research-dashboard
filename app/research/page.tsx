...
export default function TrendingPage() {
  ...
  const handleRefresh = () => {
    setLastRefresh(new Date().toLocaleString('zh-HK'));
    ...
    <div style={{ minHeight: "100vh", background: theme.bg, fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: "1400px", margin: "0 auto", padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px", marginBottom: "24px" }}>
          <KPICard label="總項目數" value={kpis.totalProjects} change="+3" icon="📊" delay={0} />
          <KPICard label="總星數" value={`${(kpis.totalStars / 1000).toFixed(0)}k`} change="+12.5k" icon="⭐" delay={1} />
          <KPICard label="每週增長" value={`+${kpis.weeklyGrowth.toLocaleString()}`} change="+8%" icon="📈" delay={2} />
          <KPICard label="熱門類別" value={kpis.topCategory} icon="🏆" delay={3} />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "24px", marginBottom: "24px" }}>
          ...
          {/* Trending Chart */}
          <Card title="📈 GitHub 星數趨勢 (頭10名)" delay={4}>
            ...
            <div style={{ height: "280px" }}>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={competitors}>
                  ...
                  <Radar name="評分" dataKey="score" stroke={theme.primary} fill={theme.primary} fillOpacity={0.3} />
                  <Radar name="功能" dataKey="features" stroke={theme.secondary} fill={theme.secondary} fillOpacity={0.3} />
                  <Radar name="社區" dataKey="community" stroke={theme.accent} fill={theme.accent} fillOpacity={0.3} />
                  <Tooltip />
                </RadarChart>
              </ResponsiveContainer>
            </div>
      ...
      <footer style={{ textAlign: "center", padding: "24px", color: theme.textSecondary, fontSize: "12px" }}>
        <p>MADHORSE Ltd. 研究儀表板 | 數據來源: COO 市場研究 | © 2026</p>
      </footer>
    </div>
  );
}
